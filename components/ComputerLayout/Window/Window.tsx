...
const BaseWindow = ({
  ...
  title = "Sans titre",
  onClose = () => { },
}: WindowProps, ref: Ref<HTMLDivElement>) => {
    ...
    <>
      {show && (
        <S.Container style={{
          top: `${pos.y}px`,
          left: `${pos.x}px`,
        }} size={size} mode={mode} followMouse={followMouse.current}>
          <S.topBar onMouseDown={() => { followMouse.current = true }} onMouseUp={() => { followMouse.current = false }}>
